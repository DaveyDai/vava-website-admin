<template>
  <div v-loading.fullscreen.lock="loading" class="product-edit">
    ...
    <div class="admin-page-card" style="width: 1200px;">
...
// import VueUpload from './web-upload-file.vue'
// import VueWebuploader from './vue-webuploader/page.vue'
export default {
  ...
  methods: {
    async querySelectData() {
      ...
      if (this.$route.query.copy) {
        this.productDetail = JSON.parse(window.sessionStorage.getItem('copyData') || {})
        window.sessionStorage.removeItem('copyData')
      }
    },
    queryProductData(id) {
      this.loading = true
    ...
    checkProductData() {
      ...
      let isPass = 0
      if (this.$refs.productFaq && !this.$refs.productFaq.checkProductFaq()) {
      ...
      this.$refs.productBaseForm.validate((valid) => {
        if (valid) {
          ...
          console.log('录入数据:', param)
          this.$confirm(param.id ? '是否确定更新该产品数据？' : '是否确定发布该产品？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
    ...
    saveProductData(data) {
      ...
      this.$store.dispatch('fetchAllList', { api: data.id ? 'updateProductInfo' : 'saveProductInfo', data: data }).then(data => {
        ...
        this.isEdit = false
        this.loading = false
        const view = this.$store.state.tagsView.visitedViews.filter(item => item.name === 'productAdd')[0]
        if (view) {
